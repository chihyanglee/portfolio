...
- **One `overrides` entry** in `pnpm-workspace.yaml` forces `smol-toml` to `^1.8.0`; Astro's `@astrojs/internal-helpers` still pins the advisory-affected 1.6.0. Delete the override once Astro bumps it — check with `pnpm why smol-toml`. `pnpm audit` should report zero; treat any new finding as signal.
